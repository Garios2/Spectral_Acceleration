...
def main(dataset: str, arch_id: str, loss: str, opt: str, lr: float, max_steps: int, mode: str="global_scaling",  neigs: int = 0, 
    ...
    if acc_goal == 0.99:
        directory = f"{directory}/warmup_beta_0.99/acc_0.99"
    else:
        directory = f"{directory}/warmup_beta_0.99"
    ...
    pretrained_dict = torch.load("/data/users/zhouwenjie/workspace/Spectral_Acceleration/results/cifar10/fc-tanh/seed_0/mse/polyak/lr_0.004_beta_0.9/warmup_beta_0.99/acc_0.9/snapshot_global_scaling_1.0_top_1")
   ...
   # param_flow = torch.zeros(max_steps // eig_freq if eig_freq >= 0 else 0, len_of_param)
    ...
    for step in range(max_steps): 
        ...
        print(f"current:{mode}\t{scaling}\t{step}\t{train_loss[step]:.3f}\t{train_acc[step]:.3f}\t{test_loss[step]:.3f}\t{test_acc[step]:.3f}beta\t{beta}")
